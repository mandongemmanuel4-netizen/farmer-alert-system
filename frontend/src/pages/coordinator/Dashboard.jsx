...
import CsoTopBar from '../../components/coordinator/CsoTopBar';
...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-official-light/20 dark:bg-gray-950 pb-24 md:ml-64">
      <CsoSidebar />
      <CsoTopBar title={`CSO ${user?.fullName || ''}`} subtitle="Good Morning," />
      <div className="md:hidden bg-official text-white px-6 pt-8 pb-6 rounded-b-3xl">
      ...
      <div className="px-6 -mt-4 md:max-w-4xl md:mx-auto md:pt-8">
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm p-4">
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500">Active Farmers</p>
          </div>
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm p-4">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm p-4">
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500">Resolved Today</p>
          </div>
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm p-4">
            <p className="text-2xl font-bold text-official">{data?.securityPosts ?? '...'}</p>
            <p className="text-xs text-gray-400 dark:text-gray-500">Security Posts</p>
          </div>
        </div>

        <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm p-4">
          <div className="flex items-center justify-between mb-3">
            <p className="font-semibold text-gray-800 dark:text-gray-100">Recent Alerts</p>
            <Link to="/coordinator/alerts" className="text-official text-sm">View All</Link>
          </div>
          {!data?.recentAlerts?.length ? (
            <p className="text-gray-400 dark:text-gray-500 text-sm">No alerts yet.</p>
          ) : (
            <div className="space-y-3">
              {data.recentAlerts.map((a) => (
                <Link key={a._id} to={`/coordinator/alerts/${a._id}`} className="flex items-center justify-between text-sm">
                  <span className="text-gray-700 dark:text-gray-200">{a.farmer?.fullName || 'Unknown farmer'}</span>
